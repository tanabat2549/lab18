...
int main() {
    ...
    while (getline(student_file, textline)) {
        ...
        int id;
        double gpa;
        char name_buffer[100]; // Temporary buffer for the name

        // Using sscanf to parse the data
        sscanf(textline.c_str(), "%99[^,], %d, %c, %lf", name_buffer, &id, &gender, &gpa);
        s.name = string(name_buffer);  // Assign the parsed name to the student
        s.id = id;
        s.gender = gender;
        s.gpa = gpa;
